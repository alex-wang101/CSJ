...
def main():
    ...
    # Find all files starting with 'AM_geocoded_' and ending with '.csv'
    am_files = glob.glob('AM_geocoded_*.csv')

    if not am_files:
        print("No files found matching 'AM_geocoded_*.csv'")
        return

    for target_file in am_files:
        # Output file: 'distances_' + everything after 'AM_geocoded_'
        base_name = os.path.basename(target_file)
        suffix = base_name[len('AM_geocoded_'):]
        output_file = f'distances_{suffix}'
